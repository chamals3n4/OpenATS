...
describe("Assessment Integration (API-level)", () => {
  beforeEach(() => {
    state.jobs = [];
    state.stages = [];
    state.assessments = [];
  ...
  it("integrates assessment creation with job-stage attachment flow", async () => {
    ...
    const stage = await request(app).post(`/api/jobs/${jobId}/pipeline`).send({
      name: "Assessment Round",
      stageType: "assessment",
    });
    const triggerStageId = Number(stage.body?.data?.id);

    const assessment = await request(app).post("/api/assessments").send({
      title: "QA Basics",
      description: "Core QA concepts",
      timeLimit: 30,
      passScore: 70,
    });
    const assessmentId = Number(assessment.body?.data?.id);

    const attach = await request(app)
      .post(`/api/jobs/${jobId}/assessments`)
      .send({ assessmentId, triggerStageId });

    expect(attach.status).toBe(201);
    expect(attach.body?.data?.jobId).toBe(jobId);
    expect(attach.body?.data?.assessmentId).toBe(assessmentId);
    expect(attach.body?.data?.triggerStageId).toBe(triggerStageId);

    const list = await request(app).get(`/api/jobs/${jobId}/assessments`);
    expect(list.status).toBe(200);
    expect(list.body?.data).toHaveLength(1);
    expect(list.body?.data[0]?.triggerStageId).toBe(triggerStageId);
  });

  it("sets assessment creator from authenticated user when createdBy is omitted", async () => {
    const assessment = await request(app).post("/api/assessments").send({
      title: "Creator Check Assessment",
      description: "Verify createdBy fallback",
      timeLimit: 20,
      passScore: 60,
    });

    expect(assessment.status).toBe(201);
    expect(assessment.body?.data?.title).toBe("Creator Check Assessment");
    expect(assessment.body?.data?.createdBy).toBe(1);
  });
});
